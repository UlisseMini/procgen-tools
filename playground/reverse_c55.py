...
def find_cheese(seed=IntSlider(min=0, max=100, step=1, value=0)):
    venv = get_cheese_venv_pair(seed=seed)
    visualize_venv(venv, mode='numpy', flip_numpy=False)

    cheese_row, cheese_col = get_cheese_pos_from_seed(seed)
    print(cheese_row, cheese_col)
    chan_row, chan_col = get_channel_from_grid_pos((cheese_row, cheese_col), layer=default_layer)
    patches = get_channel_pixel_patch(layer_name=default_layer, channel=55, coord=(chan_row, chan_col), value=5.6) # TODO set this back to zero

    fig, axs, info = compare_patched_vfields(venv, patches, hook, render_padding=True, ax_size=6)
    plot_pixel_dot(axs[1], row=chan_row, col=chan_col)
    plot_pixel_dot(axs[1], row=cheese_row, col=cheese_col, color='yellow') # Cheese location
...
def random_channel_patch(seed=IntSlider(min=0, max=100, step=1, value=0), layer_name=Dropdown(options=labels, value=default_layer), channel=channel_slider):
    """ Replace the given channel's activations with values from a randomly sampled observation. This invokes get_random_patch from patch_utils. If channel=-1, then all channels are replaced. """
