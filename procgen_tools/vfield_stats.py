...
def _decision_square_index(vf):
    return vf["legal_mouse_positions"].index(
        metrics.decision_square(vf["grid"])
    )


def _pathfind(grid: np.ndarray, start, end):
    cost, came_from, extra = maze.shortest_path(
...
def get_decision_probs(vf, return_dict: bool = False):
    "From the decision square, return probabilities of going to the cheese or top right."

    grid = vf["grid"]
    i = _decision_square_index(vf)
    probs_dict = {
        k: v for k, v in zip(models.MAZE_ACTION_INDICES.keys(), vf["probs"][i])
    }
    if return_dict:
        return probs_dict
    action_delta = models.MAZE_ACTION_DELTAS[
        max(probs_dict, key=probs_dict.get)
    ]
    ...
    #     raise ValueError('filtering by mazes where the mouse has to go
    #     down/left to get to the top right')
...
def get_decision_probs_original_and_patched(vfields, coeff: float):
    ...
    for vfs in vfields:
        if (
            vfs["coeff"] == coeff
            and metrics.decision_square(vfs["original_vfield"]["grid"])
            is not None
        ):
            decision_probs_original.append(
                get_decision_probs(vfs["original_vfield"])
            )
            decision_probs_patched.append(
                get_decision_probs(vfs["patched_vfield"])
            )
    return np.array(decision_probs_original), np.array(decision_probs_patched)


def plot_decision_probs(
    decision_probs_original, decision_probs_patched, ax_size: int = 4
):
...
def plot_dprobs_hist(dpo, dpp, ax=None):
    ...
    for i in range(3):
        ax[i].set_title(
            [f"P({label} | decision-square)" for label in _plot_labels][i]
        )
...
def get_probs_original_and_patched(
    vfields: List[dict], coeff: float
) -> Tuple[np.ndarray, np.ndarray]:
    """Get the original and patched decision probabilities from vfields, given a cheese vector coefficient."""
    dprobs_original, dprobs_patched = get_decision_probs_original_and_patched(
        vfields, coeff=coeff
    )
    probs_original, probs_patched = [
        np.stack(
            [dprobs[:, 0], dprobs[:, 1], 1 - dprobs[:, 0] - dprobs[:, 1]],
            axis=1,
        )
        for dprobs in (dprobs_original, dprobs_patched)
...
def histogram_plotly(
    coeff: float, vfields: List[dict], fig: go.Figure = plotly_fig_gen()
):
    ...
    probs_original, probs_patched = get_probs_original_and_patched(
        vfields, coeff=coeff
    )
    ...
    for i in range(3):
        ...
        fig.add_trace(
            go.Scatter(
                ...
                marker_color="blue",
                hovertext=[
                    f'seed: {vfields[idx]["seed"]}, (original'
                    f" {probs_original[idx,i]:.3f}, patched"
                    f" {probs_patched[idx,i]:.3f})"
                    for idx in range(len(probs_original))
                ],
                opacity=0.6,
            ),
            row=1,
...
def boxplot_plotly(
    ...
    for i in range(3):
        ...
        fig.add_trace(
            go.Box(
                y=probs_original[:, i], name="original", marker_color="blue"
            ),
            row=1,
            col=i + 1,
        )
        fig.add_trace(
            go.Box(
                y=probs_patched[:, i], name="patched", marker_color="orange"
            ),
            row=1,
            col=i + 1,
        )
